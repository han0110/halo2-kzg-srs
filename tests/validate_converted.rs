...
use halo2_kzg_srs::{
    ...
    util::{
        ec_point_raw_size, ec_point_repr_size, field_repr_size, perpetual_powers_of_tau, pse,
        snarkjs,
    },
};
...
async fn fetch_pse_g1s<const RAW: bool>(source: &str, k: usize, max_k: usize) -> Vec<G1Affine> {
    let uri = format!("https://trusted-setup-halo2kzg.s3.eu-central-1.amazonaws.com/{source}-{k}");
    let length = pse::G1_OFFSET as usize
        + (if RAW {
            ec_point_raw_size::<G1Affine>()
        } else {
            ec_point_repr_size::<G1Affine>()
        } << k.min(max_k));
    let mut reader = Cursor::new(fetch(&uri, 0, length).await);
    pse::read_g1s::<Bn256, _, RAW, false>(&mut reader, k.min(max_k))
}
...
async fn validate_converted_hermez() {
    ...
    for k in 1..=26 {
        let g1s = fetch_pse_g1s::<false>("hermez", k, 16).await;
        assert!(same_ratio::<Bn256>(&g1s, g2, s_g2));
    }
}

#[tokio::test]
async fn validate_converted_hermez_raw() {
    let [g2, s_g2] = fetch_hermez_g2s().await;
    for k in 1..=26 {
        let g1s = fetch_pse_g1s::<true>("hermez-raw", k, 16).await;
        assert!(same_ratio::<Bn256>(&g1s, g2, s_g2));
    }
}
...
async fn validate_converted_perpetual_powers_of_tau() {
    ...
    for k in 1..=26 {
        let g1s = fetch_pse_g1s::<false>("perpetual-powers-of-tau", k, 16).await;
        assert!(same_ratio::<Bn256>(&g1s, g2, s_g2));
    }
}

#[tokio::test]
async fn validate_converted_perpetual_powers_of_tau_raw() {
    let [g2, s_g2] = fetch_perpetual_powers_of_tau_g2().await;
    for k in 1..=26 {
        let g1s = fetch_pse_g1s::<true>("perpetual-powers-of-tau-raw", k, 16).await;
        assert!(same_ratio::<Bn256>(&g1s, g2, s_g2));
    }
}
